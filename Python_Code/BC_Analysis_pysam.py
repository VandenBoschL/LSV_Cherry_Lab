...
def BC_counts(path, counts_dict):
    with pysam.FastxFile(path) as fq:
        for record in fq:
            read = str(record.sequence).upper()
...
def main():
    ...
    cDNA_Barcodes = {barcode: 0 for barcode in table['Barcode']}
    if args.cdna:
        for fq in args.cdna:
            # Process barcodes to main counts dictionary
            BC_counts(fq, cDNA_Barcodes)

    pDNA_Barcodes = {barcode: 0 for barcode in table['Barcode']}
    if args.pdna:
        for fq in args.pdna:
            # Process barcodes to main counts dictionary
            BC_counts(fq, pDNA_Barcodes)
